...
#seeds = [100]
seeds = np.arange(1, 999, 1)
vseeds = [100, 300, 800, 900]

#############################

suff = 'pad0-pixinv-loglrelu'
...
def get_meshes(seed, galaxies=False, inverse=True):
    ...
    mesh['cic'] = np.load(path + ftypefpm%(bs, nc, seed) + 'd.npy')
    mesh['ciclog'] = np.log(1e-5 + mesh['cic'])

    hmesh = {}
    hmesh['pnn'] = np.load(path + ftypefpm%(bs, nc, seed) + 'p.npy')
    ...
    def _module_fn():
        ...
        for i in range(10):
            ul.append(PixelCNN3Dlayer(i, ul[i], f_map=f_map, full_horizontal=True, h=None, 
                                      conditional_im=conditional_im, cfilter_size=cfilter_size, gatedact='leaky_relu'))
...
#             high=2.**3-1)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        with tf.control_dependencies(update_ops):
            ...
            boundaries = list(np.array([100, 500, 1e3, 5e3, 1e4, 2e4, 3e4, 4e4]).astype(int))
            values = [1e-3, 1e-3, 1e-3, 5e-4, 5e-4, 1e-4, 5e-5, 1e-5, 1e-6]
