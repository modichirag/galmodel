...
R0 = 20
suffix = 'nc%dnorm-pixinvScicnomeanf8mix4_lr0p001/'%(resnorm)
...
#if sampleinit : suffix = suffix[:-1] + '-sampleinit/'
...
##
        ...
        for i in range(len(gradandvars_chisq)):
            ...
            smwts = tf.exp(-R0*(kmesh/(2*np.pi/bs*nc)))
        ...
        def checkiter(mode, optfolder, R0=0):
            ...
            dg.makefig(literals['truemeshes'], [meshs, meshf, meshd], optfolder+'%s.png'%(mode), boxsize=bs, title='%s'%title)
            print('Checked iter')
