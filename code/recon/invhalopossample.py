###Use the inverse network to get a sample of the final matter density and reconstruct from that 
...
def generate_sample(data, modulepath):
    ...
    with tf.Session() as sess:
        ...
        print(dataexp.shape)
        preds = sess.run(samples, feed_dict={yy:dataexp})
...
if __name__=="__main__":
    ...
    halodata = tools.paintnn(hposd, bs, nc)
    data = generate_sample(halodata, modpath)
    ...
    np.save(ofolder + '/halodata.f4', halodata)

    ###
    #Do reconstruction here
    print('\nDo reconstruction\n')

    recong = rmods.graphdm(config, data, sigma=sigma, maxiter=maxiter, anneal=anneal, gtol=gtol)    
    #
