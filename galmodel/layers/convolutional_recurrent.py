...
"""Convolutional-recurrent layers for 3D fields.
Adapted from https://github.com/tensorflow/tensorflow/blob/r1.14/tensorflow/python/keras/layers/convolutional_recurrent.py
"""
...
class ConvRNN3D(RNN):
  ...
  Input shape:
    ...
    `(samples, timesteps, channels, rows, cols, z)`
    if data_format='channels_first' or 5D tensor with shape:
    `(samples, timesteps, rows, cols, z, channels)`
  ...
  Output shape:
    - If `return_state`: a list of tensors. The first tensor is
      ...
      `(samples, filters, new_rows, new_cols, new_z)`
      if data_format='channels_first'
      or 5D tensor with shape:
      `(samples, new_rows, new_cols, new_z, filters)`
      if data_format='channels_last'.
      `rows` and `cols` values might have changed due to padding.
    - If `return_sequences`: 6D tensor with shape:
      `(samples, timesteps, filters, new_rows, new_cols, new_z)`
      if data_format='channels_first'
      or 5D tensor with shape:
      `(samples, timesteps, new_rows, new_cols, new_z, filters)`
      if data_format='channels_last'.
...
class ConvLSTM3DCell(DropoutRNNCellMixin, Layer):
  """Cell class for the ConvLSTM3D layer.
  ...
  def call(self, inputs, states, training=None):
    ...
    (kernel_i, kernel_f,
     kernel_c, kernel_o) = array_ops.split(self.kernel, 4, axis=-1)
    (recurrent_kernel_i,
     recurrent_kernel_f,
     recurrent_kernel_c,
     recurrent_kernel_o) = array_ops.split(self.recurrent_kernel, 4, axis=-1)
...
class ConvLSTM3D(ConvRNN3D):
  ...
  Input shape:
    - If data_format='channels_first'
        6D tensor with shape:
        `(samples, time, channels, rows, cols, z)`
    - If data_format='channels_last'
        6D tensor with shape:
        `(samples, time, rows, cols,z, channels)`

  Output shape:
    - If `return_sequences`
       - If data_format='channels_first'
          6D tensor with shape:
          `(samples, time, filters, output_row, output_col, output_z)`
       - If data_format='channels_last'
          6D tensor with shape:
          `(samples, time, output_row, output_col, output_z, filters)`
    - Else
      - If data_format ='channels_first'
          5D tensor with shape:
          `(samples, filters, output_row, output_col, output_z)`
      - If data_format='channels_last'
          5D tensor with shape:
          `(samples, output_row, output_col, output_z, filters)`
